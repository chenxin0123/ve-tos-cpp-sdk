...
std::map<std::string, std::string> SignV4::signHeader(const std::shared_ptr<TosRequest>& req) {
    ...
    std::string sig =
            this->doSign(req->getMethod(), req->getPath(), contentSha256, signedHeader, signedQuery, now, cred);
...
std::map<std::string, std::string> SignV4::signQuery(const std::shared_ptr<TosRequest>& req,
    ...
    std::string sig =
            this->doSign(req->getMethod(), req->getPath(), unsignedPayload, signedHeader, signedQuery, now, cred);
