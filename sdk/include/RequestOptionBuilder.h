...
public:
    ...
    void withStorageClass(const StorageClassType& storageClass) {
    ...
    void withCopySourceRange(int64_t start, int64_t end) {
        copySourceRange_.setStart(start);
        copySourceRange_.setEnd(end);
        headers_[VolcengineTos::HEADER_COPY_SOURCE_RANGE] = copySourceRange_.toString();
    }
    ...
    [[gnu::deprecated]] void withMetadataDirective(const std::string& directive) {
        headers_[VolcengineTos::HEADER_METADATA_DIRECTIVE] = directive;
    }
    void withMetadataDirective(const MetadataDirectiveType& directive) {
        headers_[VolcengineTos::HEADER_METADATA_DIRECTIVE] = MetadataDirectiveTypetoString[directive];
    }
    [[gnu::deprecated]] void withACL(const std::string& acl) {
        headers_[VolcengineTos::HEADER_ACL] = acl;
    }
    void withACL(const ACLType& acl) {
        headers_[VolcengineTos::HEADER_ACL] = ACLTypetoString[acl];
    }
    ...
    void withAzRedundancy(const AzRedundancyType& azRedundancy) {
        headers_[VolcengineTos::HEADER_AZ_REDUNDANCY] = AzRedundancyTypetoString[azRedundancy];
    }
